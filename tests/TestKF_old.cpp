...
struct PlotContainer {
  TH1D meas_x_res;
  TH1D meas_y_res;
  TH1D meas_theta_x_res;
  TH1D meas_theta_y_res;
  TH1D smooth_x_res;
  TH1D smooth_y_res;
  TH1D smooth_p_res;
  TH1D smooth_tan_res;
  ...
  TH1D h_gpos_x_distribution;
  TH1D h_gpos_y_distribution;
  TH1D h_gang_x_distribution;
  TH1D h_gang_y_distribution;
  TH1D deltaP_over_P;
  TH1D chi2;
  ...
  PlotContainer()
    : meas_x_res("meas_x_res", "meas_x_res; meas_x_res; (x_{meas} - x_{true}) [mm]; Entries", 200, -2.0, 2.0),
      ...
      meas_theta_x_res("meas_theta_x_res", "meas_theta_x_res; meas_theta_x_res; (theta_x_{meas} - theta_x_{true}) [mm]; Entries", 200, -0.2, 0.2),
      meas_theta_y_res("meas_theta_y_res", "meas_theta_y_res; meas_theta_y_res; (theta_y_{meas} - theta_y_{true}) [rad]; Entries", 200, -0.2, 0.2),
      ...
      smooth_p_res("smooth_p_res", "smooth_p_res; (p_{smoothed} - p_{true}) [MeV]; Entries", 200, -200, 200),
      ...
      h_gpos_x_distribution("h_gpos_x_distribution", "Innovation on position", 100, -3, 3),
      h_gpos_y_distribution("h_gpos_y_distribution", "Innovation on position", 100, -3, 3),
      h_gang_x_distribution("h_gang_x_distribution", "Innovation on direction", 100, -3, 3),
      h_gang_y_distribution("h_gang_y_distribution", "Innovation on direction", 100, -3, 3),
...
void tryCompleteManager(sand_reco::kf::utils::TrackletMap z_to_tracklets, SParticleInfo particle, 
  ...
  if (track.getSteps().size() > 5) { // was commented, with > 3
    ...
    for (uint i = 0; i < track.getSteps().size(); i++) {
      ...
      yz_predicted->SetPoint(i, step.getZ(), prediction.y());
      yz_filtered->SetPoint(i, step.getZ() , filtering.y());
      yz_smoothed->SetPoint(i, step.getZ() , smoothing.y());
      yz_measured->SetPoint(i, step.getZ() , step.getY());
      xz_predicted->SetPoint(i, step.getZ(), prediction.x());
      xz_filtered->SetPoint(i, step.getZ() , filtering.x());
      xz_smoothed->SetPoint(i, step.getZ() , smoothing.x());
      ...
      const auto& pred = step.getPrediction();   // TMatrixD 2x1
      ...
      const double pred_pos  = pred(0,0); 
      const double pred_ang  = pred(1,0); 
          
      const auto& orientation = step.getOrientation();
      if (orientation == sand_reco::kf::Orientation::kVertical) {
        // Piano x
        const double res_x = pred_pos - meas_pos;
        const double res_theta_x = pred_ang - meas_ang;
        plot_container.meas_x_res.Fill(res_x);
        plot_container.meas_theta_x_res.Fill(res_theta_x);
      } else {
        // Piano y
        const double res_y = pred_pos - meas_pos;
        const double res_theta_y = pred_ang - meas_ang;
        plot_container.meas_y_res.Fill(res_y);
        plot_container.meas_theta_y_res.Fill(res_theta_y);
      }
      ...
      const double res_smooth_p = (smooth_p - true_step_mom);  // MeV
      ...
      const double sigma_x     = (smooth_cov_matrix.GetNrows() > 0 && smooth_cov_matrix.GetNcols() > 0) ? std::sqrt(smooth_cov_matrix(0,0)) : 0.0;
      const double sigma_y     = (smooth_cov_matrix.GetNrows() > 1 && smooth_cov_matrix.GetNcols() > 1) ? std::sqrt(smooth_cov_matrix(1,1)) : 0.0;
      ...
      if (orientation == sand_reco::kf::Orientation::kVertical) {
        // Piano x
        plot_container.h_gpos_x_distribution.Fill(innovation[0]);
        plot_container.h_gang_x_distribution.Fill(innovation[1]);
      } else {
        // Piano y
        plot_container.h_gpos_y_distribution.Fill(innovation[0]);
        plot_container.h_gang_y_distribution.Fill(innovation[1]);
      }
...

{
  ...
  for (const auto& container:clusters.getContainers()) {
    ...
    for (const auto& cluster_in_container:container->getClusters()) {
      ...
      TVector3 true_pos = true_tracklet.pos_; // mm
      TVector3 true_dir = true_tracklet.dir_;
      TVector3 true_mom = true_tracklet.mom_; // MeV
      ...
      measurement_from_true_tracklet.x = true_pos.X(); //  + rand.Gaus() * SANDTrackerUtils::getSigmaPositionMeasurement();
      measurement_from_true_tracklet.y = true_pos.Y(); //  + rand.Gaus() * SANDTrackerUtils::getSigmaPositionMeasurement();
  ...
  for (auto trj:primaryTrj) {
    ...
    std::cout << "NUMBER OF HITS " << trj.GetHitMap().at(string_to_component[tracker_name]).size() << std::endl;
    ...
    pi.id       = trj.GetId();
    pi.mass     = particle->Mass() * sand_reco::conversion::GeV_to_MeV;
    // std::cout << " MASS: " << pi.mass << std::endl;
    ...
    for (auto& point : trj.GetTrajectoryPoints().at(string_to_component[tracker_name])) {
      if (point.GetPosition().Z() > max_z && point.GetMomentum().Z() > 100) {
        ...
        pi.pos = point.GetPosition().Vect(); // mm
        pi.mom = point.GetMomentum(); // MeV
    ...
    // pi.pos.Print();
    // std::cout << "pi.mom: " << std::endl;
    // pi.mom.Print();

    if (!to_be_reconstructed) continue;

    double sigma_pos = 0.; //rand.Gaus(0, SANDTrackerUtils::getSigmaPositionMeasurement()); // mm
    double sigma_mom = 0.;
...
int main(int argc, char* argv[])
{
  ...
  if (!plots) {
    ...
    plot_container.meas_theta_x_res.Write();
    plot_container.meas_theta_y_res.Write();
    ...
    plot_container.h_gpos_x_distribution.Write();
    plot_container.h_gpos_y_distribution.Write();
    plot_container.h_gang_x_distribution.Write();
    plot_container.h_gang_y_distribution.Write();
