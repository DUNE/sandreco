...
class BVH{
  public:
    BVH(){};
    BVH(std::vector<sand_geometry::tracker::cell_map_iterator>& cells, SANDGeoManager* geo, double max_distance) {fillCellAABBMap(cells, geo); createTree(root_, cells.begin(), cells.end(), geo); searchAdjacentCells(root_, root_, geo, max_distance);};
    ...
    void searchAdjacentCells(std::unique_ptr<Node>& node, std::unique_ptr<Node>& other_node, SANDGeoManager* geo, double max_distance);
