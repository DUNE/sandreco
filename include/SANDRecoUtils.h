...
class Helix
{
 public:
  ...
  // dip : dip angle
  // Phi0 : is the azimuth angle of the starting point with respect to the helix
  // axis
  // h : +/- sense of rotation
  // x0 : track starting pofint

  // optionsl : low_lim, up_lim
  // lower and upper limit for linear parameter s to be used in case we want
  // restrict the helix to a portion

  Helix(double arg_R, double arg_dip, double arg_Phi0, int arg_h,
        TVector3 arg_x0, double low_lim = -999., double up_lim = -999.)
      : R_(arg_R),
        dip_(arg_dip),
        Phi0_(arg_Phi0),
        h_(arg_h),
  ...
  Helix(const TG4Trajectory& trj)
  {
    ...
    h_ = (charge < 0) ? 1 : -1;
    Phi0_ = TMath::ATan2(momentum.Y(), momentum.Z()) +
            h_ * TMath::Pi() * 0.5;  // for mu pi/2 should be added

    up_lim_ = 0.;  // track starts from s = low_lim to s = 0 (for s<0 tracks
                   // proceed upwards according to this parmetrization)
    low_lim_ = -1e3;
  }
  ...
  std::vector<TVector3> GetHelixPoints(double s_min = -1e3, double s_max = 1e3,
                                       double step = 10.) const
  {
  ...
  void SetHelixRangeFromDigit(const dg_wire& digit)
  {
    ...
    double Phi_min = GetPhiFromZ(
        z_max);  // Phi is decreasing function of z (goes as acos(z))
  ...
  void PrintHelixPars() const
  {
    ...
    std::cout << std::setprecision(8) << "x0, y0, z0 -> " << x0_.X() << " "
              << x0_.Y() << " " << x0_.Z() << "\n";
    std::cout << std::setprecision(8) << "zc, yc     -> "
              << x0_.Z() - R_ * cos(Phi0_) << " " << x0_.Y() - R_ * sin(Phi0_)
              << "\n";
    std::cout << "\n";
  }
  ...
  {
    return {x0_.Z() - R_ * cos(Phi0_), x0_.Y() - R_ * sin(Phi0_)};
  };
...
class Spiral2D
{
  ...
  */
 public:
  Spiral2D(double arg_center_x, double arg_center_y, double arg_R0,
           double arg_k)
...
class Line : public SANDWireInfo
{
 public:
  ...
  // usefull as we are interest in a portion of the line defined by the wire
  // length

  Line(double arg_dx, double arg_dy, double arg_dz, double arg_ax,
       double arg_ay, double arg_az, double low_lim = -1e6, double up_lim = 1e6)
      : dx_(arg_dx),
        dy_(arg_dy),
        dz_(arg_dz),
        ax_(arg_ax),
  ...
  void PrintLineInfos()
  {
    std::cout << "line X0 : (" << ax() << ", " << ay() << ", " << az() << ")\n";
    std::cout << "line direction : (" << dx() << ", " << dy() << ", " << dz()
              << ")\n";
    std::cout << "line limits t : (" << LowLim() << ", " << UpLim() << ")\n";
  }
...
double GetDistHelix2LineDerivative(const Helix& helix, double s,
                                   const Line& line);  // may be can be remover

double GetLinePointDistance(const Line& l, TVector3 point);

double GetLineLineDistance(const Line& l1, const Line& l2,
                           double& closest2line2, double& closest2line1);

double GetSegmentSegmentDistance(const Line& l1, const Line& l2,
                                 double& closest2line2, double& closest2line1);

double GetImpactParameter(const Helix& helix, const Line& line, double s,
                          double t);
...
double GetMinImpactParameter(const Helix& helix, const Line& line,
                             double& s_min, double& t_min, bool& HasMinimized);

double NewtonRaphson2D(TF1* f, TF1* fprime, double& x_guess, double tol,
...
double NLL(Helix& h, const std::vector<dg_wire>& digits);  // negative log
                                                           // likelihood
                                                           // function
...
Line2D GetTangetTo3Circles(const Circle& c1, const Circle& c2,
                           const Circle& c3);  // bug to be fixed
