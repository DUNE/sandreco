...
public:
  SANDTrackerModule() {}
  SANDTrackerModule(long i) { _id = i; }
  ...
  void id(const long idModule) { _id = idModule; }
  long id() const { return _id; }

  void target(const std::string tagetName) { _target = tagetName; }
  std::string target() const { return _target; }

  void addPlane(const SANDTrackerPlane plane);
  SANDTrackerPlane &getPlane(int index);

  int nPlanes() const { return _vPlanes.size(); }
  std::vector<SANDTrackerPlane>& planes() {return _vPlanes;};
