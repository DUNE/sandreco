  ...
  for (std::map<std::string, std::vector<TG4HitSegment> >::iterator it =
    ...
    if (it->first == "EMCalSci") {
      for (unsigned int j = 0; j < it->second.size(); j++) {
        ...
        if (g != NULL && (digitization::ecal::edep_sim::process_hit(
          ...
          double ave_pe1 =
              digitization::ecal::edep_sim::energy_to_photo_electrons(en1);
          double ave_pe2 =
              digitization::ecal::edep_sim::energy_to_photo_electrons(en2);
          ...
          for (int i = 0; i < pe1; i++) {
            pe this_pe;
            this_pe.time = photo_electron_time_to_pmt_arrival_time(t0, d1);
            this_pe.h_index = j;
          ...
          for (int i = 0; i < pe2; i++) {
            pe this_pe;
            this_pe.time = photo_electron_time_to_pmt_arrival_time(t0, d2);
            this_pe.h_index = j;
                   ...
                   std::vector<dg_cell>& vec_cell)
{
  ...
  digitization::ecal::edep_sim::simulate_photo_electrons(ev, geo, photo_el, L);
  if (debug) {
    std::cout << "TimeAndSignal" << std::endl;
  }
  eval_adc_and_tdc_from_photo_electrons(photo_el, ps);
  if (debug) {
    std::cout << "CollectSignal" << std::endl;
  }
  group_pmts_in_cells(geo, ps, L, vec_cell);
}
  ...
  for (std::map<std::string, std::vector<TG4HitSegment> >::iterator it =
    ...
    if (it->first == "EMCalSci") {
      for (unsigned int j = 0; j < it->second.size(); j++) {
        ...
        if (g == NULL && (digitization::ecal::fluka::process_hit(
          ...
          double ave_pe1 =
              digitization::ecal::fluka::energy_to_photo_electrons(en1);
          double ave_pe2 =
              digitization::ecal::fluka::energy_to_photo_electrons(en2);
                   ...
                   std::vector<dg_cell>& vec_cell)
{
  ...
  digitization::ecal::fluka::simulate_photo_electrons(ev, geo, photo_el, L);
  if (debug) {
    std::cout << "TimeAndSignal" << std::endl;
  }
  eval_adc_and_tdc_from_photo_electrons(photo_el, ps);
                 ...
                 std::vector<dg_tube>& digit_vec)
{
  ...
  create_digits_from_hits(hits2Tube, digit_vec);
}
}  // namespace edep_sim
...
void digitize(const char* finname, const char* foutname)
{
  ...
  for (int i = 0; i < nev; i++) {
    ...
    digitization::ecal::edep_sim::digitize_ecal(ev, geo, vec_cell);
    digitization::stt::edep_sim::digitize_stt(ev, geo, digit_vec);
...
void digitize(const char* finname, const char* foutname)
{
  ...
  for (int i = 0; i < nev; i++) {
    ...
    // digitize ECAL and STT
    digitization::ecal::fluka::digitize_ecal(ev, geo, vec_cell);
    digitization::stt::fluka::digitize_stt(ev, geo, NHits, DetType, xHits, yHits,
                                        zHits, digit_vec);
