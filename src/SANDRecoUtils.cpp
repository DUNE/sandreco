...
std::vector<double> RecoUtils::SmearVariable(double mean, double sigma,
                                             int nof_points)
{
...
double RecoUtils::GetDistHelix2Line(const Helix& helix, double s,
...
double RecoUtils::GetDistHelix2LineDerivative(const Helix& helix, double s,
                                              const Line& line)
{
...
double RecoUtils::GetLineLineDistance(const Line& l1, const Line& l2,
                                      double& closest2line2,
                                      double& closest2line1)
{
  /*
                                            ...
                                            double& closest2line1)
{
  ...
  */
  ...
  d = (l1.GetPointAt(seg_closest2line2) - l2.GetPointAt(seg_closest2line1))
          .Mag();
...
double RecoUtils::GetImpactParameter(const Helix& helix, const Line& line,
                                     double s, double t)
{
                                        ...
                                        bool& HasMinimized)
{
  ...
  for (auto i = 0u; i < nof_helix_sampling_points; i++) {
    ...
    //               << ", "                         << ((dist2wire <
    // impact_parameter)&&((line.GetPointAt(t_i)-line.GetLinePointX0()).Mag()))
    //               <<"\n";

    if ((dist2wire < impact_parameter) &&
        ((line.GetPointAt(t_i) - line.GetLinePointX0()).Mag() <=
         line.GetLineLength() / 2.)  // is within SAND x range
        ) {
                                  ...
                                  double tol, unsigned int max_iterations)
{
  /*
      ...
      Return f(x_sol)
  */
  // std::cout << "x_guess " << x_guess << "f->Eval(x_guess) "<<
  // f->Eval(x_guess) << "\n";
...
double RecoUtils::GetMinImpactParameter(const Helix& helix, const Line& line)
{
  ...
  double t_min = -999;
  bool HasMinimized = 0;
  return RecoUtils::GetMinImpactParameter(helix, line, s_min, t_min,
                                          HasMinimized);
}
...
TF1* RecoUtils::InvertSin(TF1* fSin)
{
  ...
  */
  ...
  // std::cout << "function [x_min, x_max] = [" << fSin->GetXmin() << ", " <<
  // fSin->GetXmax() << "]\n";
  // std::cout << "function [z_min, z_max] = [" << fSin->Eval(fSin->GetXmin())
  // << ", " << fSin->Eval(fSin->GetXmax()) << "]\n";
  // std::cout << "inverse function [z_min, z_max] = [" << z_min << ", " <<
  // z_max << "]\n";

  // TF1* fAsin = new TF1("fAsin", "(( asin((x - [0])/[1]) - [2] ) / [3] )",
  // z_min, z_max);
  TF1* fAsin =
      new TF1("fAsin", "((asin((x-[0])/[1]) + 2*TMath::Pi()*[2])-[3])/[4]");
...
TF1* RecoUtils::WiresSinFit(const std::vector<dg_wire>& wires)
{
  ...
  */
  ...
  double initial_offset_guess =
      std::accumulate(z.begin(), z.end(), 0.0) / z.size();
  ...
  sineFit->SetParameters(initial_amplitude_guess, initial_frequency_guess,
                         initial_phase_guess, initial_offset_guess);
...
Line2D RecoUtils::WiresLinearFit(const std::vector<dg_wire*>& wires)
{
  ...
  */
  ...
  graph->Fit(linearFit,
             "Q");  // "Q" option suppresses the fit statistics output
...
double RecoUtils::NLL(Helix& h, const std::vector<dg_wire>& digits)
{
  ...
  for (auto& digit : digits) {
    ...
    // nll += (r_estimated - r_measured) * (r_estimated - r_measured) / (sigma *
    // sigma);
...
double RecoUtils::GetDipAngleFromCircleLine(const Circle& circle,
                                            const Line2D& line, double Phi0,
                                            int helicity, TVector3& momentum)
{
  /*
      input:
          ...
          Momentum : reference TVector3 that will be filled with particle
                     total momentum
      NOTE: a guess for the initial vertex is needed because related to the dip
     angle
  */
                                        ...
                                        TVector3& momentum)
{
  ...
  */
  ...
  double dip_angle = RecoUtils::GetDipAngleFromCircleLine(circle, line, Phi0,
                                                          helicity, momentum);
                                            ...
                                            int& TMinuitStatus)
{
  ...
  minimizer->SetLimitedVariable(1, "dip", helix_initial_guess.dip(), 0.01, -1.6,
                                1.6);
  // minimizer->SetLimitedVariable(2,    "Phi0", helix_initial_guess.Phi0(),
  // 0.01,  -3.14, 3.14);
  // minimizer->SetLimitedVariable(3,    "h",    helix_initial_guess.h(), 0.01,
  // -1.1,  1.1);
  minimizer->SetLimitedVariable(4, "x0_x", helix_initial_guess.x0().X(), 1,
                                -1800, 1800);
  // minimizer->SetLimitedVariable(5,    "x0_y", helix_initial_guess.x0().Y(),
  // 1,  -4500, -300);
  // minimizer->SetLimitedVariable(6,    "x0_z", helix_initial_guess.x0().Z(),
  // 1,  21500, 26000);
...
std::vector<double> CardanoSolutions(double a, double b, double c, double d)
{
  ...
  f = ((3 * c / a) - ((b * b) / (a * a))) / 3;
  g = ((2 * (b * b * b) / (a * a * a)) - (9 * b * c / (a * a)) + (27 * d / a)) /
      27;
  h = ((g * g) / 4) + ((f * f * f) / 27);
  std::cout << "h : " << h << "\n";
  if (f == 0 && g == 0 && h == 0) {
    ...
    x1 = s + u - b / (3 * a);
    x2 =
        std::complex<double>((s + u) / 2, -(s - u) * sqrt(3) / 2) - b / (3 * a);
...
std::vector<Line2D> RecoUtils::GetTangentsTo2Circles(const Circle& c1,
                                                     const Circle& c2)
{
  /*
...
Line2D RecoUtils::GetTangetTo3Circles(const Circle& c1, const Circle& c2,
                                      const Circle& c3)
{
  // !! bug to be fixed
...
{
  if (circles.size() < 4) {
    std::cout << "too few circles to fit in function " << __FILE__ << ", "
              << __LINE__ << "\n";
    std::cout << "try use RecoUtils::GetTangetTo3Circles \n";
    throw "";
  }
  auto guess_tangents =
      RecoUtils::GetTangentsTo2Circles(circles[0], circles[1]);
  ...
  for (auto& tangent : guess_tangents) {
    ...
    for (auto i = 2u; i < circles.size(); i++) {
      double expected_radius = tangent.Distance2Point(
          {circles[i].center_x(), circles[i].center_y()});
      double measured_radius = circles[i].R();
      tangent_score += (expected_radius - measured_radius) *
                       (expected_radius - measured_radius) / sigma * sigma;
    }
