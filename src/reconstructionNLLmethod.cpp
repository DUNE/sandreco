...
void help_input()
{
  ...
  std::cout
      << "--signal_propagation : include signal_propagation in digitization \n";
  std::cout << "--hit_time           : include hit time in digitization \n";
  std::cout << "--track_no_smear     : reconstruct non smeared track (NO "
               "E_LOSS NO MCS)\n";
  std::cout << "--debug              : use higher verbosity for TMinuit " << def
            << std::endl;
}
...
bool PassSelectionNofHits(const std::vector<dg_wire>& fired_wires)
{
  ...
  */
  LOG("i",
      TString::Format(
          "Requiring at least %d hits on ZY bending plane and %d on XZ plane",
          MIN_NOF_ZY_HITS, MIN_NOF_XZ_HITS).Data());
  int nof_ZY_hits = 0;
  int nof_XZ_hits = 0;
...
//     /*
//         select events in the sand fiducial volume at 10 cm far from the
// frames:
//         - 5 cm along x from the frame edge;
//     */
//     auto vertex_position = vertex.GetPosition();
//     bool pass_x = (fabs(vertex_position.X() - SAND_CENTER_X) <=
// SAND_TRACKER_X_LENGTH/2. - 50);
//     // get supermod number
//     int smod = (vertex_position.Z() -
// SAND_TRACKER_Z_START)/SAND_SUPEMOD_Z_THICK;
//     // event should be 5 cm far from supermod frame
//     bool pass_zy = (fabs(vertex_position.Y() - SAND_CENTER_Y) <=
// SUPERMOD_LENGTHS[smod]/2. - 50);
//     return pass_x * pass_zy;
// }

std::string GetVolumeFromCoordinates(std::string units, double x, double y,
                                     double z)
{
...
bool IsInSMODFiducialVol(int smod, double x, double y, double z)
{
  ...
  bool pass_x =
      (fabs(x - SAND_CENTER_X) <= SAND_TRACKER_X_LENGTH / 2. - FIDUCIAL_CUT);
  bool pass_zy =
      (fabs(y - SAND_CENTER_Y) <= SUPERMOD_Y_HEIGHT[smod] / 2. - FIDUCIAL_CUT);
  return pass_x * pass_zy;
}
...
void CreateDigitsFromHelix(Helix& h, std::vector<dg_wire>& wire_infos,
                           std::vector<dg_wire>& fired_wires)
{
  /*
      - run over wires
      - set helix range
      - call RecoUtils::GetMinImpactParameter(const Helix& helix, const Line&
     line)
  ...
  */
  ...
  double last_s_min = 0.;
  //
  // scan all the wires TDC and select only the onces with the closest impact
  // par
  for (auto& w : wire_infos) {
    ...
    /* define point on the helix and on the line corresponding to the impact
       parameter
       t_min is how much do I move from the wire center to get to the line point
       closest to the helix.
       It has a sign.
    */
    double s_min, t_min;
    ...
    double impact_par =
        RecoUtils::GetMinImpactParameter(h, l, s_min, t_min, HasMinimized);

    if ((impact_par <= SENSE_2_SENSE_DIST * 0.5 * sqrt(2)) &
        (fabs(t_min) * l.GetDirectionVector().Mag() <=
         w.wire_length)) {  // conditions for fired_wire
      ...
      if (INCLUDE_SIGNAL_PROPAGATION) {
        ...
        w.signal_time =
            (t_min + w.wire_length / 2.) / sand_reco::stt::v_signal_inwire;
        w.tdc += w.signal_time;
      }

      if (INCLUDE_HIT_TIME) {
        if (last_s_min !=
            0)  // for first fired hit hit_timer=NEUTRINO_INTERACTION_TIME
          hit_timer += (h.GetPointAt(s_min) - h.GetPointAt(last_s_min)).Mag() /
                       LIGHT_VELOCITY;
        w.t_hit = hit_timer;
        w.tdc += w.t_hit;
        last_s_min = s_min;
...
std::vector<TG4HitSegment> FilterHits(const std::vector<TG4HitSegment>& hits,
                                      const int PDG)
{
  /*
  ...
  */
  ...
  for (auto& hit : hits) {
    ...
    for (auto& id : hit.Contrib) {
      if ((evEdep->Trajectories[id].GetPDGCode() == PDG) &&
          (evEdep->Trajectories[id].GetParentId() == -1)) {
        contribution = true;
        break;
      }
    }
...
{
  // sort fired wires by hit time (MC truth): usefull to subract assumed t_hit
  // from measured TDC
  std::sort(
      wires.begin(), wires.end(),
      [](const dg_wire& w1, const dg_wire& w2) { return w1.t_hit < w2.t_hit; });
}
  ...
  */
  ...
  for (auto& hit : muon_hits) {
    for (auto& wire : wire_infos) {
      ...
      bool is_hit_in_wire_plane =
          fabs(wire.z - hit_middle.Z()) < MYLAR_2_MYLAR_DIST * 0.5;

      if (is_hit_in_wire_plane) {  // pass if hit z coodinate is found in the
                                   // wire plane
        ...
        double hit_2_wire_dist = RecoUtils::GetSegmentSegmentDistance(
            wire_line, hit_line, closest_2_hit, closest_2_wire);
        ...
        bool is_hit_in_cell =
            (wire.hor == true)
                ? fabs(w_point.Y() - h_point.Y()) < SENSE_2_SENSE_DIST * 0.5
                : fabs(w_point.X() - h_point.X()) <= SENSE_2_SENSE_DIST * 0.5;

        if (is_hit_in_cell) {  // // pass if hit y (or x) coodinate is found in
                               // the hor (or vertical) wire cell

          auto fired = RecoUtils::Copy(wire);

          fired.drift_time = hit_2_wire_dist / sand_reco::stt::v_drift;

          fired.signal_time = (w_point - wire_line.GetLineUpperLimit()).Mag() /
                              sand_reco::stt::v_signal_inwire;

          fired.t_hit = ((hit.GetStop() + hit.GetStart()) * 0.5 +
                         (hit.GetStop() - hit.GetStart()) * closest_2_wire).T();
...
Parameter CreateParam(std::string name, int id, bool fixed_in_fit,
                      double initial_guess, double value, double error)
{
  Parameter p;
  p.name = name;
  p.id = id;
...
void GetTrackFirstGuess(Circle& circle, Line2D& line)
{
  ...
  */
  ...
  LOG("R",
      TString::Format("first guess : Center = (%f, %f), R =  %f",
                      circle.center_x(), circle.center_y(), circle.R()).Data());

  // LOG("R", TString::Format("track vertex (x, y, z) : (%f, %f, %f)", x0.X(),
  // x0.Y(), x0.Z()).Data());
  // LOG("R", TString::Format("Momentum (px, py, pz) : (%f, %f, %f)",
  // momentum.X(), momentum.Y(), momentum.Z()).Data());
...
double GetMissingCoordinate(dg_wire& vertical_wire, const Circle& track_guess)
{
  ...
  dg_wire closest_horizontal =
      FindClosestZWire(vertical_wire, horizontal_fired_digits);
...
void TDC2ImpactPar(dg_wire& wire, const T& track_guess)
{
  ...
  */
  ...
  if (INCLUDE_SIGNAL_PROPAGATION) {
    ...
    */
    if (wire.hor == true) {  // horizontal
      ...
      Line wire_line = RecoUtils::GetLineFromWire(wire);
      wire.signal_time_measured =
          (signal_origin_on_wire - wire_line.GetLineUpperLimit()).Mag() /
          sand_reco::stt::v_signal_inwire;
    } else {  // vertical
  ...
  wire.drift_time_measured =
      wire.tdc - wire.signal_time_measured - wire.t_hit_measured;
}
  ...
  for (auto& wire : fired_wires) {
    ...
    if (_DEBUG_) {
      std::cout << "wire id " << wire->did << " is horizontal : " << wire->hor
                << "\n";
      LOG("R", TString::Format("SIGNAL time: true %f, measured %f",
                               wire->signal_time,
                               wire->signal_time_measured).Data());
      std::cout << "\n";
    }
  }
}
...
std::vector<Circle> Wire2DriftCircle(const std::vector<dg_wire*>& wires)
{
  ...
  for (const auto& wire : wires) {
    if (wire->hor == true) {  // horizontal
      Circle c(wire->z, wire->y,
               wire->drift_time_measured * sand_reco::stt::v_drift);
      drift_cirlces.push_back(c);
    } else {  // vertical
      Circle c(wire->x, wire->z,
               wire->drift_time_measured * sand_reco::stt::v_drift);
      drift_cirlces.push_back(c);
    }
  }
...
double FunctorNLL_Circle(const double* p)
{
  ...
  for (auto& wire : horizontal_fired_digits) {
    ...
    nll += (r_estimated - r_observed) * (r_estimated - r_observed) /
           (sigma * sigma);
...
Circle FitZYDriftCircles(Circle& first_guess, MinuitFitInfos& fit_infos)
{
  /*
...
double FunctorNLL_Line(const double* p)
{
  ...
  for (auto& wire : vertical_fired_digits) {
    ...
    //           << ", impact par : true " << wire.drift_time *
...
Line2D FitXZDriftCircles(Line2D first_guess, MinuitFitInfos& fit_infos)
{
  /*
  ...
  */
  ...
  // fill output object fit_infos
  Parameter m =
      CreateParam("m", 0, false, first_guess.m(), pars[0], parsErrors[0]);
  Parameter q =
      CreateParam("q", 1, false, first_guess.q(), pars[1], parsErrors[1]);
...
void FitDriftCircles(Circle& circle_ZY_plane, Line2D& line_XZ_plane,
                     MinuitFitInfos& fit_ZY, MinuitFitInfos& fit_XZ)
{
...
double GetImpactParameterSin(TF1* TestSin, const dg_wire& wire)
{
  ...
  */
  ...
  TF1* TestSin_prime =
      new TF1("TestSin_prime", "[0]*[1]*cos([1]*x + [2])", x_min, x_max);
  ...
  /*
      ...
      - parameter (x_wire, z_wire) coordinate f the wire to which the distace
        is calculated
  */
  TF1* distance_sin_wire = new TF1(
      "distance_sin_wire", [&](double* x, double* par) {
                             double sin_at_x = TestSin->Eval(x[0]);
                             double x_w = par[0];
                             double z_w = par[1];
                             return sqrt((x[0] - x_w) * (x[0] - x_w) +
                                         (sin_at_x - z_w) * (sin_at_x - z_w));
                           },
      x_min, x_max, 2);
  distance_sin_wire->SetParameters(x_wire, z_wire);

  // derivative of distance
  TF1* distance_sin_wire_prime = new TF1(
      "distance_sin_wire_prime",
      [&](double* x, double* par) {
        ...
        double z_w = par[1];
        return 1. / (2. * distance) *
               (2 * (x[0] - x_w) + 2 * (sin_at_x - z_w) * derivative_at_x);
      },
      x_min, x_max, 2);
  ...
  double impact_par = RecoUtils::NewtonRaphson2D(
      distance_sin_wire, distance_sin_wire_prime, x_guess, 0.2, 1000);
...
double NLL_Sin(TF1* TestSin, std::vector<dg_wire*>& wires)
{
  ...
  */
  ...
  for (auto& wire : wires) {
    ...
    // std::cout << "impact par : true " << wire.drift_time *
    // sand_reco::stt::v_drift
    //                << ", r_observed : " << r_observed
    //                << ", r_estimated : " << r_estimated << "\n";
  }
...
double FunctorNLL_Sin(const double* p)
{
  ...
  TestSin->SetParameters(amplitude, frequency, phase, offset);

  return NLL_Sin(TestSin, vertical_fired_digits);
}

TF1* GetRecoXZTrack(TF1* first_guess, MinuitFitInfos& fit_infos)
{
  /*
  ...
  */
  ...
  // A
  minimizer->SetLimitedVariable(0, "A", amplitude, amplitude * 0.01,
                                amplitude * 0.8, amplitude * 1.2);
  // B
  minimizer->SetLimitedVariable(1, "B", frequency, frequency * 0.01,
                                frequency * 0.8, frequency * 1.2);
  // C
  // minimizer->SetLimitedVariable(2, "C", phase, phase*0.01, phase*0.8,
  // phase*1.2);
  // D
  // minimizer->SetLimitedVariable(3, "D", offset, offset*0.01, offset*0.8,
  // offset*1.2);
  ...
  // fill output object fit_infos
  Parameter A =
      CreateParam("amplitude", 0, false, amplitude, pars[0], parsErrors[0]);
  Parameter B =
      CreateParam("frequency", 1, false, frequency, pars[1], parsErrors[1]);
  ...
  // final fit
  TF1* SinFinalFit =
      new TF1("SinFinalFit", "[0]*sin([1]*x + [2]) + [3]", x_min, x_max);
...
Helix GetRecoHelix(Helix& helix_initial_guess,
                   std::vector<dg_wire>& fired_wires, MinuitFitInfos& fit_infos,
                   int FitStrategy = 1)
{
  /*
  ...
  */
  ...
  minimizer->SetLimitedVariable(0, "R", helix_initial_guess.R(), 10,
                                helix_initial_guess.R() * 0.8,
                                helix_initial_guess.R() * 1.2);  // R fit range

  minimizer->SetLimitedVariable(
      1, "dip", helix_initial_guess.dip(), 0.01,
      helix_initial_guess.dip() * 0.8,
      helix_initial_guess.dip() * 1.2);  // dip fit range

  minimizer->SetLimitedVariable(
      4, "x0_x", helix_initial_guess.x0().X(), 1,
      helix_initial_guess.x0().X() - 20,
      helix_initial_guess.x0().X() + 20);  // x0 fit range
  ...
  // fill output object fit_infos
  Parameter R_ = CreateParam("R", 0, false, helix_initial_guess.R(), pars[0],
                             parsErrors[0]);
  Parameter dip_ = CreateParam("dip", 1, false, helix_initial_guess.dip(),
                               pars[1], parsErrors[1]);
  Parameter Phi0_ = CreateParam("Phi0", 2, true, helix_initial_guess.Phi0(),
                                pars[2], parsErrors[2]);
  Parameter h_ = CreateParam("h", 3, true, helix_initial_guess.h(), pars[3],
                             parsErrors[3]);
  Parameter x0_ = CreateParam("x0", 4, false, helix_initial_guess.x0().X(),
                              pars[4], parsErrors[4]);
  Parameter y0_ = CreateParam("y0", 5, true, helix_initial_guess.x0().Y(),
                              pars[5], parsErrors[5]);
  Parameter z0_ = CreateParam("z0", 6, true, helix_initial_guess.x0().Z(),
                              pars[6], parsErrors[6]);
  ...
  Helix reco_helix(pars[0], pars[1], pars[2], pars[3],
                   {pars[4], pars[5], pars[6]});

  return reco_helix;
}

Helix Reconstruct(Circle FittedCircle, Line2D FittedLine,
                  ...
                  const std::vector<dg_wire*>& ver_wires)
{
  ...
  */
  ...
  auto track_direction_ZY =
      FittedCircle.GetDerivativeAt(vertex_ZY.X(), vertex_ZY.Y()).Unit();
  ...
  */
  ...
  LOG("R", TString::Format("Reconstructed muon (px, py, pz) : (%f, %f, %f)", px,
                           py, pz).Data());
...
Helix Reconstruct(MinuitFitInfos& fit_infos, Helix& true_helix,
                  Helix& helix_first_guess, std::vector<dg_wire>& wire_infos,
                  std::vector<dg_wire>& fired_wires)
{
  ...
  std::cout << "dip_true - dip_reco : " << true_helix.dip() - reco_helix.dip()
            << "\n";
  std::cout << "x0_true - x0_reco   : " << true_helix.x0().X() -
                                               reco_helix.x0().X() << "\n";
...
int main(int argc, char* argv[])
{
  ...
  /*
     ...
     (for testing used file
     "/storage/gpfs_data/neutrino/users/gi/sand-reco/tests/test_drift_edep.root")
  */
  const char* fEDepInput;

  /*
     provide digit file input to run the reconstruction on edep smeared muon
     tracks
  */
  ...
  /*
     look up table previously created in the Digitization as csv file that
     contains
     all info about wires in the geometry
     for testing used file
     "/storage/gpfs_data/neutrino/users/gi/sand-reco/tests/wireinfo.txt"
  */
  ...
  std::cout << "Signal propagation..."
            << (INCLUDE_SIGNAL_PROPAGATION ? "enabled" : "disabled")
            << std::endl;
  std::cout << "Hit time............."
            << (INCLUDE_HIT_TIME ? "enabled" : "disabled") << std::endl;
  std::cout << "Debug mode..........." << (_DEBUG_ ? "enabled" : "disabled")
            << std::endl;
  ...
  for (auto i = 0u; i < tEdep->GetEntries(); i++) {
    ...
    LOG("I", "Test muon");
    std::cout << "event number : " << i << ", muon_momentum : ("
    ...
    bool event_in_fiducial_volume =
        IsInFiducialVolume("mm", vertex.GetPosition().X(),
    ...
    // create the non-smeared track (no E loss nor MCS) from initial muon
    ...
    if (USE_NON_SMEARED_TRACK) {
      ...
      CreateDigitsFromEDep(evEdep->SegmentDetectors[trackerType.c_str()],
                           wire_infos, fired_wires);
    }
    ...
    for (auto cycle = 0u; cycle < nof_cycles; i++) {
      LOG("I", TString::Format("---------> Starting cycle number %d ", cycle)
                   .Data());
      ...
      for (auto w : *RecoUtils::event_digits)
        deviation += (w.signal_time - w.signal_time_measured);
      LOG("W", TString::Format(
                   "Scarto totale tempi di segnale veri e quelli assunti %f",
                   deviation).Data());

      LOG("I", "Creating drift circles from drift time");
      std::vector<Circle> vertical_drift_circles =
          Wire2DriftCircle(vertical_fired_digits);
      std::vector<Circle> horizontal_drift_circles =
          Wire2DriftCircle(horizontal_fired_digits);
    ...
    reco_helix = RecoUtils::GetHelixFromCircleLine(
        circle_ZY_plane, line_XZ_plane, reco_helix.x0(), particle_momentum);
    reco_helix.PrintHelixPars();

    LOG("R", TString::Format("Reconstruncted total momentum: (%f, %f, %f)",
                             particle_momentum.X(), particle_momentum.Y(),
                             particle_momentum.Z()));
    ...
    reco_object.p_true = {muon_trj.GetInitialMomentum().X(),
                          muon_trj.GetInitialMomentum().Y(),
                          muon_trj.GetInitialMomentum().Z()};
    reco_object.p_reco = {particle_momentum.X(), particle_momentum.Y(),
                          particle_momentum.Z()};
...
// LOG("R", TString::Format("Tangent to first TDC XZ plane m = %f, q = %f",
// segmentsXZ[0].m(), segmentsXZ[0].q()).Data());
...
// LOG("R", TString::Format("Tangent to first TDC XZ plane m = %f, q = %f",
// segmentsZY[0].m(), segmentsZY[0].q()).Data());