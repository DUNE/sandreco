...
// ST front : 'frontST([0-9]+)_vol_(hor|ver)_ST_stGas_([a-zA-Z]{2})19_vol_PV_([0-9]+)'
// ST module: 'sttmod([0-9]+)_ST_(hor|ver)_ST_stGas_([a-zA-Z]{2})19_vol_PV_([0-9]+)'
// both: '(sttmod|frontST)([0-9]+)_(ST|vol)_(hor|ver)_ST_stGas_([a-zA-Z]{2})19_vol_PV_([0-9]+)'
...
void getSTPlaneinfo(TGeoNode* nod, TGeoHMatrix mat, std::map<int, std::map<double, int> >& stX, std::map<int, std::map<int, TVector2> >& stPos, std::map<int, double>& slabPos)
{
  TString name = nod->GetName();
...
void showElemPos(TGeoNode* nod, TGeoHMatrix mat, TString path)
{
  ...
  if((name.Contains("stGas_Ar19_vol_PV_") || name.Contains("stGas_Xe19_vol_PV_")) && !name.Contains("coat") && !name.Contains("wire") && !name.Contains("air") && !name.Contains("mylar"))
  {
    TObjArray* obj = name.Tokenize("_");
    ...
    std::cout << name.Data() << " -> mod: " << mod << " type: " << type << " id: " << id << " x(y): " << x << " z: " << z << std::endl;
  } 
...
void showElemPos()
{
  ...
  getSTPlaneinfo(gGeoManager->GetTopVolume()->GetNode(0), mat, stX, stPos, slabPos);
  ...
  {
    for(std::map<int, TVector2>::iterator iter = it->second.begin(); iter != it->second.end(); ++iter)
    {
      std::cout << it->first << " " << iter->first << " " << iter->second.X() << " " << iter->second.Y() << std::endl;
    }
  }*/
