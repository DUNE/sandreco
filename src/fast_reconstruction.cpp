    ...
    if (ev->Trajectories[jj].TrackId == trackid) {
      for (unsigned int kk = 0; kk < ev->Trajectories[jj].Points.size() - 1;
        ...
        std::cout << ev->Trajectories[jj].Points[kk].Position.X() << " " <<
                     ...
                     ev->Trajectories[jj].Points[kk+1].Position.Z() << " " <<
                     x << " " << y << " " << z << " " << dpos.back() << " " <<
                     ev->Trajectories[jj].TrackId << " " <<
        ev->Trajectories[jj].PDGCode << std::endl;
        */
...
void TrackFit(std::vector<track>& vec_tr)
{
  ...
  for (unsigned int j = 0; j < vec_tr.size(); j++) {
    ...
    while((n_v < int(z_v.size())) && (z_v[n_v-1]-z_v[n_v-2])*(z_v[1]-z_v[0]) >
    0.)
      n_v++;
    ...
    for(int k = 0; k < n_v; k++)
    {
      ...
      {
        double dz2 = vec_tr[j].r*vec_tr[j].r - (z_v[k] - vec_tr[j].zc)*(z_v[k] -
    vec_tr[j].zc);
        ...
        //{
        //  std::cout << vec_tr[j].yc << " " << vec_tr[j].h << " " << dz <<
    std::endl;
        //}
        n_v_fit++;
      }
    ...
    */
    ...
    double dz2 = vec_tr[j].r*vec_tr[j].r - (z0 - vec_tr[j].zc)*(z0 -
    vec_tr[j].zc);
    double dz = dz2 < 0. ? 0. : TMath::Sqrt(dz2);
...
//smearing functions from Bing
//
void smearPar(int trackid, std::string name){


        if(name=="mu-" || name=="mu+" || name=="proton" || name=="pi+" || name=="pi-") std::cout<<"smearChargedPar(trackid)"<<std::endl;
        else if(name=="e-" || name=="e+") std::cout<<"smearChargedPar(trackid)"<<std::endl;
        else if(name=="kaon+" || name=="kaon-") std::cout<<"smearChargedPar(trackid)"<<std::endl;
        else if(name=="pi0") std::cout<<"smearPi0(trackid)"<<std::endl;
        else if(name=="neutron") std::cout<<"smearNeutron(trackid)"<<std::endl;
        else if(name=="gamma") std::cout<<"smearGamma(trackid)"<<std::endl;
        else if(name=="kaon0" || name=="anti_kaon0" || name=="kaon0L") std::cout<<"smearDaughters(trackid)"<<std::endl;
        else if(name=="lambda" || name=="anti_lambda")  std::cout<<"smearDaughters(trackid)"<<std::endl;   // lambda travel very short distance (5cm) then decay to pi- and proton
        else if(name=="sigma+" || name=="sigma-") std::cout<<"smearSTT_or_Daughters(trackid)"<<std::endl; // sigma+/- can create medium track then decay
        else if(name=="sigma0") std::cout<<"smearDaughters(trackid)"<<std::endl;  // sigma0 decay right at vertex to 1 lambda and 1 gamma
        else if(name=="eta") std::cout<<"smearDaughters(trackid)"<<std::endl; //eta decay right there to 3 pi0s or other modes
        else if(name=="anti_proton") std::cout<<"smearChargedPar(trackid)"<<std::endl;
        ...
        else if(name=="anti_neutron") std::cout<<"smearNeutron(trackid)"<<std::endl;
        else {
             //   std::cout<<"--->####################################### unknown par:"<<name<<"  iEntry:"<<iEntry<<" trackid:"<<trackid<<std::endl;  //FIXME..to be modified
...
void smearNeutron(int trackid){

std::cout<<"Smearing of neutron to be implemented"<<std::endl;
}
...
//FIXME this part must be copied from src/analysis.cpp
  event evt;
...
//    vec_tr.clear();  //Matteo fills those, but for us they are probably useless
//    vec_cl.clear();

//////////////////////////////////////////////From Bing smearEvent

         //cleanBranch();    //FIXME to be replaced
         std::cout<<" ############################################################## new event ####################################  "<<i<<std::endl;
        //if(debug>=3)  showAll();
       // organizeHits();   //FIXME to be replaced?
       // organizeHits_prim2();  //FIXME to be replaced?
       // makeTree();    //FIXME to be replaced?
        //if(ns_Fast_Reco::debug>=2) drawTree();
        //  treefile.close();
        ns_Fast_Reco::iFillPar=0;
        int nPrim= ev->Primaries.begin()->Particles.size();

        bool havePrimNeutron=false;
        bool NeutronAtLast=false;
        int neutron_trackid;
        //if(isHtarget) havePrimNeutron=checkPrimNeutron(NeutronAtLast, neutron_trackid);  //FIXME to be included
        //if(isHtarget && havePrimNeutron && (!NeutronAtLast)) {                           //FIXME to be included 
	
		//BING SMEARING   we need the functions   smearPar    smearNeutron 

                for (int i=0;i<nPrim;i++){
                        if(ev->Primaries.begin()->Particles[i].TrackId==neutron_trackid) continue;
                        if(ns_Fast_Reco::debug>=2) std::cout<<" +++++++++++++++++++++++++++++++++++++++++++++++++++++smearPar: "<<ev->Primaries.begin()->Particles[i].Name<<" id: "<<ev->Primaries.begin()->Particles[i].TrackId<<std::endl;
                        smearPar(ev->Primaries.begin()->Particles[i].TrackId , ev->Primaries.begin()->Particles[i].Name);
                }
                if(ns_Fast_Reco::debug>=2)   std::cout<<"smear neutron at the very end since its a numubar hydrogen target event"<<std::endl;
                smearNeutron(neutron_trackid);
        //} else {   //else case of (isHtarget && havePrimNeutron && (!NeutronAtLast))   FIXME to be included
              //  for (int i=0;i<nPrim;i++){
                //        if(ns_Fast_Reco::debug>=2) std::cout<<"++++++++++++++++++++++++++++++++++++++++++++++++++smearPar: "<<ev->Primaries.begin()->Particles[i].Name<<" id: "<<ev->Primaries.begin()->Particles[i].TrackId<<std::endl;
                 //       smearPar(ev->Primaries.begin()->Particles[i].TrackId , ev->Primaries.begin()->Particles[i].Name);
               // }
       // }
        ...
        if(ns_Fast_Reco::iFillPar==0) return;

//output of Bing....these must be changed with the Matteo format
 /* 
       brNPar=iFillPar;
        brNPrim=nPrim;
 ...
 //       herr_nu_E->Fill((recoNuE-trueNuE)/trueNuE*100.); //isto of Bing  we can avoid?
