...
bool IsPrimary(TG4Event* ev, int tid)
{
  for (unsigned int i = 0; i < ev->Primaries.at(0).Particles.size(); i++) {
    if (ev->Primaries.at(0).Particles.at(i).TrackId == tid) return true;
  }
  return false;
}
...
void FillTrackInfo(track& tr, particle& p)
{
  ...
  if(p.has_track)
  {
    ...
    p.E_tr = TMath::Sqrt(p.px_tr*p.px_tr + p.py_tr*p.py_tr + p.pz_tr*p.pz_tr +
p.mass*p.mass);
    p.x_tr = tr.x0;
    p.y_tr = tr.y0;
    p.z_tr = tr.z0;
...
void RecoFromTrack(particle& p)
{
  if (p.tr.ret_ln == 0 && p.tr.ret_cr == 0) {
    ...
    */
    ...
    p.Ereco = TMath::Sqrt(p.pxreco * p.pxreco + p.pyreco * p.pyreco +
                          p.pzreco * p.pzreco + p.mass * p.mass);
    p.xreco = p.tr.x0;
    p.yreco = p.tr.y0;
    p.zreco = p.tr.z0;
    p.treco = p.tr.t0;
...
void RecoGamma(particle& p)
{
  ...
  if (p.has_daughter == 1) {
    for (unsigned int i = 0; i < p.daughters.size(); i++) {
      if (p.daughters.at(i).has_track == 1 &&
          p.daughters.at(i).tr.ret_ln == 0 &&
          p.daughters.at(i).tr.ret_cr == 0) {
        ok = true;
      }
    }
  }
...
void FillClusterInfo(TG4Event* ev, const cluster& cl, particle& p)
{
  ...
  switch(p.pdg)
  {
    ...
    case -2112: // antineutron
    {
      ...
      {
        cell_time[i] = TfromTDC(cl.cells.at(i).tdc1, cl.cells.at(i).tdc2,
cl.cells.at(i).l);

        //std::cout << cell_time[i] << " " << cl.cells.at(i).tdc1 << " " <<
cl.cells.at(i).tdc2 << " " << cl.cells.at(i).l << std::endl;
      }

      int idx_min = std::distance(cell_time.begin(),
std::min_element(cell_time.begin(), cell_time.end()));
...
cl.cells.at(idx_min).y);
      ...
      //  << cl.cells.at(idx_min).tdc1 << " " << cl.cells.at(idx_min).tdc2 << "
" << cl.cells.at(idx_min).l << " "
      //  << xmin << " " << ymin << " " << zmin << " "
      //  << ev->Primaries.at(0).Position.X() << " " <<
ev->Primaries.at(0).Position.Y() << " " << ev->Primaries.at(0).Position.Z() << "
"
      //  << ev->Primaries.at(0).Position.T() << " " << dt << " " << dr << " "
<< beta << std::endl;
      ...
      //std::cout << idx_min << " " << tmin << " " << xmin << " " << ymin << " "
<< zmin << " " << dt << " " << dx << " " << dy << " " << dz << " " << dr << " "
<< beta << " " << gamma << " " << p.mass << std::endl;
      break;
    }
    case 22: // gamma
    case 11: // electron
    ...
    case 111: // pi zero
    {
      ...
      for(unsigned int i = 0; i < cl.cells.size(); i++)
      {
        ...
        {
          xv = XfromTDC(cl.cells.at(i).tdc1, cl.cells.at(i).tdc2,
cl.cells.at(i).x);
          yv = cl.cells.at(i).y;
          zv = cl.cells.at(i).z;
        }
        ...
        {
          xv = cl.cells.at(i).x;
          yv = XfromTDC(cl.cells.at(i).tdc1, cl.cells.at(i).tdc2,
cl.cells.at(i).y);
          zv = cl.cells.at(i).z;
        }
    ...
    default: // other (assuming hadron)
    {
      ...
      for(unsigned int i = 0; i < cl.cells.size(); i++)
      {
        ...
        if(cl.cells.at(i).id < 25000)
        {
          x[cl.cells.at(i).lay] += e * XfromTDC(cl.cells.at(i).tdc1,
cl.cells.at(i).tdc2, cl.cells.at(i).x);
        ...
        else
        {
          ...
          y[cl.cells.at(i).lay] += e * XfromTDC(cl.cells.at(i).tdc1,
cl.cells.at(i).tdc2, cl.cells.at(i).y);
          z[cl.cells.at(i).lay] += e * cl.cells.at(i).z;
        }
      }
...
void EvalNuEnergy(event& ev)
{
  ...
  for (unsigned int i = 0; i < ev.particles.size(); i++) {
    if (ev.particles.at(i).primary == 1) {
      if ((ev.particles.at(i).pdg == 2212 || ev.particles.at(i).pdg == 2112) &&
          ev.particles.at(i).Ereco > 0)  // proton
      {
